...
namespace SchemaTool
{
    ...
    static class P4Tool
    {
        ...
        public static void PerforceReconcileFile(string filepath, string linkbase = null)
        {
            if (m_p4info == null)
            {
                return;
            ...
            if (JunctionPoint.Exists(linkbase))
            {
                ...
                filepath = filepath.Replace(linkbase, replace);
            }
            Process p4 = new Process();
            p4.StartInfo.FileName = "p4.exe";
            p4.StartInfo.Arguments = $"-p {m_p4info.server} -u {m_p4info.user} -P {m_p4info.passwd} -c {m_p4info.client} reconcile -aedI {filepath}";
            p4.StartInfo.UseShellExecute = false;
            p4.Start();
        }
