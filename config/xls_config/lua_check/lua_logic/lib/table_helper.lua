...
function table.tostr(t, tabnum, float) --
	...
	for i = 1, #t0 do
		...
		k = tabs .. tostring(k)
		if tv=='table' then ts[#ts+1] = k..'='..table.tostr(v, tabnum + 1,float)
...
function table.randomn( t, n )
	if #t < n then assert(false, 'cant sel n from t') end
...
function table.icontains(t, value)
	...
	for ii=1, #t do
		if t[ii] == value then return true; end
	end
	return false
end
